"""
ETH 15-minute — NORMAL mode.

Quiet stream. Trades every ~3 minutes (5 cycles per window).
Output focuses on:
  • Order placed / settled + bankroll change
  • 2-minute market digest
...
_DIGEST_EVERY = 120   # status digest every 2 minutes
_trade_num    = 0
...
def _digest(tick: PriceTick) -> None:
    ...
    secs_left = trader._market_spec.seconds_until_resolution
